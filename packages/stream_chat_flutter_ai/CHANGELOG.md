...
- **`ComposerThemeData` and `SuggestionsThemeData`**, joining `ChartThemeData` on the `AITheme`
  extension. `ChatComposer`, `ComposerAttachmentSheet` and `AISuggestionsView` previously took every
...
- `ChatComposerSlotProps` carries the composer's whole wiring — `controller`, `focusNode`, `onSend` and `onStop` — so **every** slot can drive the composer, not just the input one. A trailing send or stop button, or a leading button that returns focus to the field, is now a few lines in a factory; previously `onSend`/`onStop`/`focusNode` sat on `ChatComposerInputProps` and nothing outside the input could reach them. This matches `stream_chat_flutter`, whose composer puts its send button in the trailing slot for exactly this reason, and it is why the three non-input props subclasses add no fields of their own. `ChatComposerInputProps` keeps only what is meaningless elsewhere: the text-field configuration.
- `ChatComposerSendCallback` now returns `FutureOr<void>` instead of `void`, matching the `async` callback the docs and README have always shown. Existing callbacks are unaffected. The composer still clears the field as soon as the callback is invoked rather than when it completes — that is deliberate and now documented, since awaiting a round trip would leave the composer looking unresponsive — but a rejected future is reported through `FlutterError.onError` instead of vanishing as an unhandled asynchronous error (silent in release).
- `ChatComposerInputProps.onStop` is now `VoidCallback?` and is `null` when the host passed no `ChatComposer.onStopPressed` — **breaking change** for a custom input reading it. It previously always handed over a callable that quietly did nothing, leaving a custom input no way to tell that stopping was unsupported. The default input now renders its stop button disabled in that case.
- **A send is now refused while `ChatComposerController.isGenerating` is `true`**, unless the new `ChatComposer.allowSendWhileGenerating` is set — **breaking change in behavior** for a custom slot that sent mid-stream. The composer's own UI expresses this by showing a stop button instead of a send button, but that left the rule a property of the *default input* rather than of the composer: `buildInput` or `buildTrailing` could put a send button on screen mid-stream and it sent. Set the flag if your backend accepts a follow-up while it is still answering.
- `ChatComposerSlotProps` is now `abstract`. It has no valid instances of its own — every slot passes one of the four subclasses — so the public constructor only allowed a props object belonging to no slot.
...
- Initial implementation of `stream_chat_flutter_ai`:
  ...
  - `ChatComposerInput` — the composer's default input pill (text field, inline option chip, attachment thumbnails, morphing mic/send/stop control), returned by `ChatComposerFactory.buildInput`. Drives its own rebuilds, so it works standalone as well as inside `ChatComposer`.
  - `ChatComposerSlotProps` and its per-slot subclasses `ChatComposerLeadingProps`, `ChatComposerTrailingProps`, `ChatComposerInputProps` and `ChatComposerAttachmentSheetProps` — what each `ChatComposerFactory` slot receives.
  - `SpeechToTextButton` — microphone button that streams partial speech-to-text results into the composer's text field; replaced by the send button once the field has content, except while a dictation session is running (see 🔄 Changed).
...
- An attachment thumbnail whose file can no longer be read — deleted from the gallery after being picked, or on a volume whose permission was revoked — now renders a broken-image placeholder and reports through `FlutterError.onError`. `FutureBuilder` surfaces a read failure as a snapshot with no data, which is indistinguishable from "still loading", so it previously stayed an empty grey square forever with nothing said about why.
